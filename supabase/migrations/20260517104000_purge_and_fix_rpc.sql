-- 1. Replace the RPC to fix the worker release bug (V4.4)
...
BEGIN
    ...
    -- 1. Identify and release employees (via worker_assignments)
    ...
    -- THIS IS THE BUG FIX: update the new "employees" table via string matching as well
    ...
    END IF;
    -- (Keep legacy workers fallback just in case)
...
-- 2. PURGE DATA 
-- Remove all existing leads and clients. 
-- We will use the robust function for leads to ensure all history and assignments are also purged properly.
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN SELECT id FROM public.crm_leads LOOP
        PERFORM public.delete_crm_lead_robust(r.id);
    END LOOP;
END;
$$;

-- Since the robust function deletes matching clients, we will just delete any remaining loose clients
DELETE FROM public.clients;

-- Reset all employees just to be absolutely sure no one is stuck assigned
UPDATE public.employees SET assigned_client = NULL, status = 'available';
