...
export default function Dashboard() {
    ...
    const statCards = [
        { label: 'Active Leads', value: stats.activeLeads.value, trend: stats.activeLeads.trend, icon: <TrendingUp className="w-5 h-5 text-blue-400"/>, bg: 'bg-blue-500/20 border border-blue-500/30' },
        { label: 'Active Deployments', value: stats.activeWorkers.value, trend: stats.activeWorkers.trend, icon: <Users className="w-5 h-5 text-emerald-400"/>, bg: 'bg-emerald-500/20 border border-emerald-500/30' },
        { label: 'Platform MRR', value: stats.totalMrr.value, trend: stats.totalMrr.trend, icon: <ArrowUpRight className="w-5 h-5 text-cyan-400"/>, bg: 'bg-cyan-500/20 border border-cyan-500/30' },
        { label: 'AI Voice Calls', value: stats.aiVoiceCalls.value, trend: stats.aiVoiceCalls.trend, icon: <Phone className="w-5 h-5 text-orange-400"/>, bg: 'bg-orange-500/20 border border-orange-500/30' },
    ];
    ...
    const getActivityStyling = (eventType: string) => {
        const type = eventType.toLowerCase();
        if (type.includes('whatsapp') || type.includes('message') || type.includes('sent')) {
            return { icon: <MessageSquare className="w-4 h-4" />, bg: 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' };
        }
        if (type.includes('form') || type.includes('consent')) {
            return { icon: <FileText className="w-4 h-4" />, bg: 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' };
        }
        if (type.includes('call') || type.includes('voice')) {
            return { icon: <Phone className="w-4 h-4" />, bg: 'bg-orange-500/20 text-orange-400 border border-orange-500/30' };
        }
        return { icon: <AlertCircle className="w-4 h-4" />, bg: 'bg-blue-500/20 text-blue-400 border border-blue-500/30' };
    };
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-white font-['Plus_Jakarta_Sans']">Analytics Command Center</h1>
                    <p className="text-slate-300 mt-1">Real-time business performance and AI automation metrics.</p>
                </div>
            </div>
                ...
                {statCards.map((stat, i) => (
                    <div key={i} className="glass-card p-5 flex flex-col justify-between">
                        ...
                        <div className="flex items-end justify-between">
                            ...
                            <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-300 flex items-center gap-1 shadow-sm">
                                {stat.trend}
                            </span>
                        </div>
                    </div>
            ...
            <div className="grid lg:grid-cols-3 gap-6 lg:h-[400px]">
                {/* Revenue Chart */}
                <div className="glass-card lg:col-span-2 p-6 flex flex-col h-[300px] lg:h-auto">
                    <div className="mb-4">
                        <h2 className="font-bold text-white text-lg">Monthly Recurring Revenue (MRR)</h2>
                        <p className="text-xs text-slate-300">Projected trajectory based on active worker deployments.</p>
                    </div>
                ...
                <div className="glass-card overflow-hidden flex flex-col h-[400px] lg:h-auto">
                    <div className="p-5 border-b border-white/10 flex items-center justify-between">
                        <h2 className="font-bold text-white">Recent AI Activity</h2>
                        <span className="text-[10px] font-black tracking-widest uppercase text-white bg-white/20 px-2 py-1 rounded-md">Live Stream</span>
                    </div>
                    <div className="p-4 flex-1 overflow-auto">
                        {recentActivity.length > 0 ? (
                            <div className="space-y-3">
                                {recentActivity.map(activity => {
                                    const styling = getActivityStyling(activity.event_type);
                                    return (
                                    <div key={activity.id} className="flex gap-3 rounded-lg border border-white/10 bg-white/5 p-3">
                                        <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${styling.bg}`}>
                                            {styling.icon}
                                        </div>
                                        <div className="min-w-0 flex-1">
                                            ...
                                            {activity.leadName && (
                                                <p className="text-xs text-slate-400 mt-1 truncate">{activity.leadName}</p>
                                            )}
                                        </div>
                                    </div>
                                    );
                            ...
                            <div className="h-full flex items-center justify-center text-center">
                                <div>
                                    ...
                                    <p className="text-sm text-slate-300 font-medium">No recent activity.</p>
