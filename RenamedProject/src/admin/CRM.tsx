...
export default function CRM() {
    ...
    const sendWorkerProfileWhatsApp = async (lead: any, worker: any) => {
        try {
            ...
            if (!isRealWorker) {
                const text = `Healthcare: Namaste ${lead.name}! Aapke liye ${worker.name || worker.full_name} (${worker.role || worker.job_title}) ko assign kiya gaya hai. Dhanyawad! ✅`;
            ...
            const text = `Healthcare: Namaste ${lead.name}! Aapke liye ${worker.name || worker.full_name} (${worker.role || worker.job_title}) ko assign kiya gaya hai.\n\n🔗 Unki verified ID Card dekhein: ${result.shareableUrl}\n\nYe link 30 din tak valid hai. Dhanyawad! ✅`;
            let phone = (lead.whatsapp_number || lead.phone || '').replace(/\D/g, '') || '917575041313';
            if (phone.length === 10) phone = `91${phone}`;
            window.open(`https://wa.me/${phone}?text=${encodeURIComponent(text)}`, '_blank');
    ...
    const generateWhatsappDraft = (
        ...
        // e.g. "https://healthcare.in/staff/{{link}}" -> "{{link}}"
        if (tpl.includes('https://healthcare.in/staff/')) {
            tpl = tpl.replace(/https:\/\/healthcare\.in\/staff\//g, '');
        }
