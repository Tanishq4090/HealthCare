...
export default defineConfig(({ mode }) => {
  ...
  const pwaManifest =
    appMode === 'os'
      ? {
        name: 'Healthcare OS — Private Portal',
        short_name: 'Healthcare OS',
        description: 'Private client operations portal for Healthcare',
        ...
        start_url: '/',
        icons: [
          { src: '/healthcare-favicon.svg', sizes: '192x192', type: 'image/svg+xml', purpose: 'any maskable' },
          { src: '/healthcare-favicon.svg', sizes: '512x512', type: 'image/svg+xml', purpose: 'any maskable' },
        ],
        shortcuts: [
          { name: 'Open Dashboard', url: '/admin', description: 'Go to Healthcare OS dashboard' },
        ],
      }
      : {
