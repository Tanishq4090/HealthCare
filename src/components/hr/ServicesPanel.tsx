...
export default function ServicesPanel({ 
    ...
    const [isSendingFinalWhatsApp, setIsSendingFinalWhatsApp] = useState(false);
    const [hasSentFinalWhatsApp, setHasSentFinalWhatsApp] = useState(false);
    ...
    useEffect(() => {
        const sub = supabase
            .channel('services_panel_realtime')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'services' }, () => {
                fetchServices();
            })
            .on('postgres_changes', { event: '*', schema: 'public', table: 'service_bills' }, () => {
                fetchServices();
            })
    ...
    const handleSendFinalInvoiceWhatsApp = async () => {
        if (!endResult) return;
        setIsSendingFinalWhatsApp(true);
        const toastId = toast.loading('Dispatching final settlement invoice on WhatsApp...');
        try {
            let clientPhone = endResult.client_phone || '';
            let clientName = endResult.client_name || 'Client';

            if (!clientPhone && endResult.service_id) {
                const svc = services.find(s => s.id === endResult.service_id);
                if (svc) {
                    clientPhone = (svc.clients as any)?.phone_number || '';
                    clientName = (svc.clients as any)?.client_name || clientName;
                }
            }

            if (!clientPhone && endResult.client_id) {
                const { data: leadData } = await supabase
                    .from('crm_leads')
                    .select('phone, whatsapp_number, name')
                    .eq('id', endResult.client_id)
                    .maybeSingle();
                if (leadData) {
                    clientPhone = leadData.phone || leadData.whatsapp_number || '';
            ...
            const refundOrOwed = (endResult.settlement || 0) < 0
                ? `Refund Due to Client: ₹${Math.abs(endResult.settlement || 0).toLocaleString('en-IN')}`
                : `Balance Due: ₹${Math.max(0, endResult.settlement || 0).toLocaleString('en-IN')}`;

            const plainTextMessage = 
                `Hello ${clientName},\n\n` +
                `Your Final Settlement Tax Invoice (${endResult.invoice_number}) has been generated by 99 Care for your concluded service.\n\n` +
                `📋 Verified Service Days: ${endResult.total_lifetime_days} days\n` +
                `💰 Gross Service Cost: ₹${(endResult.true_cost || 0).toLocaleString('en-IN')}\n` +
                `🧾 Previously Billed: ₹${(endResult.previously_billed || 0).toLocaleString('en-IN')}\n` +
                `🔒 Security Deposit: ₹${(endResult.deposit || 0).toLocaleString('en-IN')}\n` +
                `📌 Final Settlement: ${refundOrOwed}\n\n` +
                (endResult.invoice_pdf_url ? `📄 View & Download Invoice PDF:\n${endResult.invoice_pdf_url}\n\n` : '') +
                `Thank you for choosing 99 Care! For assistance, please call +91 9016116564.`;

            const SUPABASE_URL = import.meta.env.VITE_SUPABASE_URL;
            const SUPABASE_ANON_KEY = import.meta.env.VITE_SUPABASE_ANON_KEY;

            let sentAutomated = false;

            if (digits && SUPABASE_URL && SUPABASE_ANON_KEY) {
                try {
                    const waResp = await fetch(`${SUPABASE_URL}/functions/v1/meta-whatsapp-outbound`, {
                        ...
                        body: JSON.stringify({
                            phone: digits,
                            leadId: endResult.client_id,
                            message: plainTextMessage,
                            sendInvoicePdf: Boolean(endResult.invoice_pdf_url),
                            invoicePdfUrl: endResult.invoice_pdf_url,
                            useTemplate: false,
                        }),
                    });
                    const waData = await waResp.json().catch(() => ({}));
                    if (waResp.ok && waData.success !== false) {
                        sentAutomated = true;
                    }
                } catch (err) {
                    console.warn('Automated WhatsApp dispatch failed, falling back to direct link:', err);
                }
            }

            if (sentAutomated) {
                toast.success(`Final settlement invoice sent directly to ${clientName} on WhatsApp!`, { id: toastId });
                setHasSentFinalWhatsApp(true);
            } else {
                const waUrl = digits
                    ? `https://api.whatsapp.com/send?phone=${digits}&text=${encodeURIComponent(plainTextMessage)}`
                    : `https://api.whatsapp.com/send?text=${encodeURIComponent(plainTextMessage)}`;
                window.open(waUrl, '_blank');
                toast.success(`Opening WhatsApp with pre-filled final invoice & PDF link!`, { id: toastId });
                setHasSentFinalWhatsApp(true);
            }
        } catch (err: any) {
            toast.error(`WhatsApp dispatch failed: ${err.message}`, { id: toastId });
        } finally {
            setIsSendingFinalWhatsApp(false);
        }
    };

    const handleSendBillWhatsApp = async (service: ServiceWithDetails, bill: any) => {
        const toastId = toast.loading('Preparing WhatsApp invoice dispatch...');
        try {
            const clientName = (service.clients as any)?.client_name || 'Client';
            let clientPhone = (service.clients as any)?.phone_number || '';
            if (!clientPhone && service.client_id) {
                const { data: leadData } = await supabase
                    .from('crm_leads')
                    .select('phone, whatsapp_number, name')
            ...
            const invoiceUrl = bill.invoice_pdf_url || '';
            const isFinal = bill.type === 'final';
            const invoiceNo = bill.invoice_number || 'INV';
            const billAmount = (bill.amount || 0).toLocaleString('en-IN');

            const plainTextMessage = isFinal
                ? `Hello ${clientName},\n\nYour Final Settlement Invoice (${invoiceNo}) of ₹${billAmount} has been generated by 99 Care.\n\n${invoiceUrl ? `📄 View Invoice PDF:\n${invoiceUrl}\n\n` : ''}Thank you for trusting 99 Care!`
                : `Hello ${clientName},\n\nYour Service Invoice (${invoiceNo}) of ₹${billAmount} has been generated by 99 Care.\n\n${invoiceUrl ? `📄 View Invoice PDF:\n${invoiceUrl}\n\n` : ''}Thank you for trusting 99 Care!`;
            ...
            if (digits && SUPABASE_URL && SUPABASE_ANON_KEY) {
                try {
                    const waResp = await fetch(`${SUPABASE_URL}/functions/v1/meta-whatsapp-outbound`, {
                        ...
                        body: JSON.stringify({
                            phone: digits,
                            leadId: service.client_id,
                            message: plainTextMessage,
                            sendInvoicePdf: Boolean(invoiceUrl),
                            invoicePdfUrl: invoiceUrl || undefined,
                            useTemplate: false,
                        }),
                    });
                    const waData = await waResp.json().catch(() => ({}));
            ...
            if (sentAutomated) {
                toast.success(`Invoice sent to ${clientName} on WhatsApp!`, { id: toastId });
            } else {
                const waUrl = digits
                    ? `https://api.whatsapp.com/send?phone=${digits}&text=${encodeURIComponent(plainTextMessage)}`
                    : `https://api.whatsapp.com/send?text=${encodeURIComponent(plainTextMessage)}`;
                window.open(waUrl, '_blank');
                toast.success(`Opening WhatsApp with pre-filled invoice message!`, { id: toastId });
            }
        } catch (err: any) {
                            ...
                            {endResult.invoice_number && (
                                <div className="p-3 bg-teal-50/80 border border-teal-200 rounded-xl flex items-center justify-between mt-2">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        <button
                                            onClick={handleSendFinalInvoiceWhatsApp}
                                            disabled={isSendingFinalWhatsApp}
                                            className="px-2.5 py-1.5 bg-[#25D366] hover:bg-[#20bd5a] text-white text-xs font-bold rounded-lg transition-colors flex items-center gap-1 shadow-xs disabled:opacity-60 cursor-pointer"
                                            title="Send invoice directly to client on WhatsApp"
                                        >
                                            {isSendingFinalWhatsApp ? (
                                                <Loader2 className="w-3.5 h-3.5 animate-spin" />
                                            ) : hasSentFinalWhatsApp ? (
                                                <Check className="w-3.5 h-3.5" />
                                            ) : (
                                                <MessageSquare className="w-3.5 h-3.5 fill-white/20" />
                                            )}
                                            {hasSentFinalWhatsApp ? 'Sent' : 'WhatsApp'}
                                        </button>
                                        {endResult.invoice_pdf_url && (
                                            <a
                            ...
                            <button
                                ...
                                className={`w-full py-2.5 px-4 rounded-xl font-bold text-xs flex items-center justify-center gap-2 shadow-sm transition-all ${
                                    hasSentFinalWhatsApp
                                        ...
                                        : 'bg-[#25D366] hover:bg-[#20bd5a] text-white shadow-emerald-600/25 hover:shadow-md'
                            ...
                            >
                                {isSendingFinalWhatsApp ? (
                                    ...
                                    <>
                                        ...
                                        <span>Resend Final Invoice on WhatsApp</span>
                                    </>
                                ) : (
                                    <>
                                        <MessageSquare className="w-4 h-4 fill-white/20" />
                                        <span>Send Final Invoice on WhatsApp</span>
                                    </>
                                )}
                            </button>
                            ...
                            <div className="flex gap-2">
                                {endResult.invoice_pdf_url && (
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="flex-1 py-2 text-xs font-bold text-center border border-slate-300 text-slate-700 rounded-xl hover:bg-slate-50 transition-colors flex items-center justify-center gap-1.5"
                                    >
                                ...
                                <button
                                    ...
                                    className="flex-1 py-2 text-xs font-bold bg-slate-900 text-white rounded-xl hover:bg-slate-800 transition-colors cursor-pointer"
                                >
                                    Done
                                </button>
                            </div>
